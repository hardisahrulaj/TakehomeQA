...
// FirstName intentionally not set (TC003: No First Name)
...
if (titleText.equals('Student Registration Form')) {
	...
	if (firstNameValue == null || firstNameValue.isEmpty()) {
		log.logPassed('Success : Field First Name kosong, form tidak terkirim')
		KeywordUtil.markPassed('Success : Field First Name kosong, form tidak terkirim')
	} else {
		log.logFailed('Failed : Field First Name terisi padahal seharusnya kosong')
		KeywordUtil.markFailed('Failed : Field First Name terisi padahal seharusnya kosong')
	}
	log.logPassed('Success : Data Form tidak terkirim')
	log.logPassed('Success : Muncul validasi mandatory pada kolom First Name')
	KeywordUtil.markPassed('Success : Data Form tidak terkirim')
	WebUI.takeScreenshot()
	KeywordUtil.markPassed('Success : Muncul validasi mandatory pada kolom First Name')
} else {
	WebUI.delay(1)
	WebUI.takeScreenshot()
