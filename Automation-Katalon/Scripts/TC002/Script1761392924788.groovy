...
// LastName intentionally not set (TC002: No Last Name)
...
if (titleText.equals('Student Registration Form')) {
	WebUI.delay(1)
	String lastNameValue = WebUI.getAttribute(findTestObject('Object Repository/Take Home Test/Page_DEMOQA/input_Name_lastName'), 'value')
	if (lastNameValue == null || lastNameValue.isEmpty()) {
		log.logPassed('Success : Field Last Name kosong, form tidak terkirim')
		KeywordUtil.markPassed('Success : Field Last Name kosong, form tidak terkirim')
	} else {
		log.logFailed('Failed : Field Last Name terisi padahal seharusnya kosong')
		KeywordUtil.markFailed('Failed : Field Last Name terisi padahal seharusnya kosong')
	}
	log.logPassed('Success : Data Form tidak terkirim')
	log.logPassed('Success : Muncul validasi mandatory pada kolom Last Name')
	KeywordUtil.markPassed('Success : Data Form tidak terkirim')
	WebUI.takeScreenshot()
	KeywordUtil.markPassed('Success : Muncul validasi mandatory pada kolom Last Name')
} else {
	WebUI.delay(1)
	WebUI.takeScreenshot()
