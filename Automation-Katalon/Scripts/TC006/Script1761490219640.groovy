...
// MobileNumber intentionally not set (TC006: No Mobile)
...
if (titleText.equals('Student Registration Form')) {
	WebUI.delay(1)
	String mobileValue = WebUI.getAttribute(findTestObject('Object Repository/Take Home Test/Page_DEMOQA/input_(10 Digits)_userNumber'), 'value')
	if (mobileValue == null || mobileValue.isEmpty()) {
		log.logPassed('Success : Field Mobile Number kosong, form tidak terkirim')
		KeywordUtil.markPassed('Success : Field Mobile Number kosong, form tidak terkirim')
	} else {
		log.logFailed('Failed : Field Mobile Number terisi padahal seharusnya kosong')
		KeywordUtil.markFailed('Failed : Field Mobile Number terisi padahal seharusnya kosong')
	}
	log.logPassed('Success : Data Form tidak terkirim')
	log.logPassed('Success : Muncul validasi pada kolom Mobile Number')
	KeywordUtil.markPassed('Success : Data Form tidak terkirim')
	WebUI.takeScreenshot()
	KeywordUtil.markPassed('Success : Muncul validasi mandatory pada kolom Mobile Number')
} else {
	WebUI.delay(1)
	WebUI.takeScreenshot()
